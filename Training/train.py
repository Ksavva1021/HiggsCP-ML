...
def main(dataset = 'dataset',
   ...
   if output_scaling:
      target_means = targets_train.mean()
      target_stds   = targets_train.std()
      targets_train = (targets_train - target_means) / target_stds
      targets_test = (targets_test - target_means) / target_stds
   ...
   loss_dict = create_lossFunction(maeLoss=True,crossProduct=True)
   ...
   train_dataset = tf_dataset(inputs_train.to_numpy(),targets_train.to_numpy(),batch_size,True,cat_train['tau_decay_mode_1_mapping'].to_numpy(),cat_train['tau_decay_mode_2_mapping'].to_numpy(), raw_inputs_train.to_numpy())
   test_dataset = tf_dataset(inputs_test.to_numpy(),targets_test.to_numpy(),batch_size,False,cat_test['tau_decay_mode_1_mapping'].to_numpy(),cat_test['tau_decay_mode_2_mapping'].to_numpy(), raw_inputs_test.to_numpy())
   # ----------------------------------------
   ...
   for epoch in range(epochs):
      ...
      for step, (input_batch_train, DM1_batch_train, DM2_batch_train, raw_inputs_batch_train, target_batch_train) in enumerate(train_dataset):
         train_losses,total_train_loss = train_step(model, optimizer,[input_batch_train, DM1_batch_train, DM2_batch_train],target_batch_train, raw_inputs_batch_train, target_stds.to_numpy(), target_means.to_numpy(), loss_dict)
      ...
      for step_val,(input_batch_test, DM1_batch_test, DM2_batch_test, raw_inputs_batch_test, target_batch_test) in enumerate(test_dataset):
         test_losses,total_test_loss = test_step(model,[input_batch_test, DM1_batch_test, DM2_batch_test],target_batch_test, raw_inputs_batch_test, target_stds.to_numpy(), target_means.to_numpy(), loss_dict)
...
# ------------Extra Utilities------------
def tf_dataset(inputs, targets, batch_size, shuffle=False, *args):
   nevents = len(inputs)
   data = (inputs,*args,targets)
...
def create_model(input_shape, output_shape, categorical_inputs, embedded_mappings, units, activation, kernel_initializer, input_scaling, *args):
   inputs = []
   models = []
   ...
   # Output layer
   outputs = tf.keras.layers.Dense(output_shape, use_bias=True, kernel_initializer = kernel_initializer, name = "output")(x)

   model = tf.keras.Model(inputs = inputs, outputs=outputs, name="regression")
   return model
     
def create_lossFunction(maeLoss=False,crossProduct=False):
   ...
   if crossProduct:
      ...
      #   # Formula: a x b / |a x b|
      #   a = tf.stack([px, py, pz], axis=-1)
      #   b_target = tf.stack([targets[:,6], targets[:,7], targets[:,8]], axis=-1)
      #   b_prediction = tf.stack([predictions[:,6], predictions[:,7], predictions[:,8]], axis=-1)
      ...
      #losses['CP_PV1'] = crossProduct_PV1

      #def crossProduct_PV2(**kwargs):
      #   targets = kwargs["targets"]
      #   predictions = kwargs["predictions"]
      #   raw = kwargs['raw_inputs']
      #   stdev = kwargs['stdev']
      ...
      #   # Formula: a x b / |a x b|
      #   a = tf.stack([px, py, pz], axis=-1)
      #   b_target = tf.stack([targets[:,10], targets[:,11], targets[:,12]], axis=-1)
      #   b_prediction = tf.stack([predictions[:,10], predictions[:,11], predictions[:,12]], axis=-1)
      ...
      #losses['CP_PV2'] = crossProduct_PV2
      ...
      #   # Formula: a x b / |a x b|
      #   a_target = tf.stack([targets[:,6], targets[:,7], targets[:,8]], axis=-1)
      #   a_prediction = tf.stack([predictions[:,6], predictions[:,7], predictions[:,8]], axis=-1) 
      #   b_target = tf.stack([targets[:,10], targets[:,11], targets[:,12]], axis=-1)
      #   b_prediction = tf.stack([predictions[:,10], predictions[:,11], predictions[:,12]], axis=-1)
...
@tf.function
def train_step(model, optimizer, x, y, z, stdev, mean, loss_fns):
    with tf.GradientTape() as tape:
        ...
        losses = {name: loss_fn(targets=y, predictions = predictions, raw_inputs = z, stdev=stdev, mean=mean) for name, loss_fn in loss_fns.items()}
        loss_value = tf.add_n(list(losses.values()))
...
@tf.function
def test_step(model, x, y, z, stdev, mean, loss_fns):
    # Forward pass: compute predictions using the model
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', "--targets", type=str, help='A list of elements in string format, e.g., "[x, y, z]"')
    ...
    if args.targets and args.save_dir:
       if not os.path.exists(args.save_dir):
          create_directory(args.save_dir)
       main(dataset=dataset,inputs=inputs,targets=targets,save_directory = args.save_dir)
    else:
       sys.exit()   
